...
st.markdown("YOLOv8 + FaceNet | UI-driven workflow")
...
st.header("1️⃣ Upload Training Images")

uploads = st.file_uploader(
    f"Upload images of {TARGET_IDENTITY}",
    accept_multiple_files=True,
    type=["jpg", "png", "jpeg"]
)

person_dir = os.path.join(FACES_DB, TARGET_IDENTITY)
...
if uploads:
    ...
    st.success(f"{len(uploads)} images uploaded")
...
st.markdown("---")
st.markdown("**Status:** Stable | Logs + Evidence fixed")
